subroutine shifte(mpmax,mp)
! this subroutine can be used for other species by changing "electron" to other species name
...
! change - wj
  if(iteration>mtoroidal.and.m0<=mp)then
     write(*,*)'shifte endless particle sorting loop at PE=',mype
...
! label particle to be moved
     do m=mbeg,mend
        zetaright=min(2.0*pi,zelectron(3,m))-zeta1
        zetaleft=zelectron(3,m)-zeta0
...
!$omp parallel do private(m)
  do m=1,msendright(1)
     ...
     sendright(nparam+1:nzphase,m)=zelectron0(1:nparam,iright(m))
  enddo
...
!$omp parallel do private(m)
  do m=1,msendleft(1)    
     ...
     sendleft(nparam+1:nzphase,m)=zelectron0(1:nparam,ileft(m))
  enddo
...
!$omp parallel do private(m)
  do m=1,mrecvleft(1)
     zelectron(1:nparam,m+mp)=recvleft(1:nparam,m)
     zelectron0(1:nparam,m+mp)=recvleft(nparam+1:nzphase,m)
  enddo

! particle moved from right
!$omp parallel do private(m)
  do m=1,mrecvright(1)
     zelectron(1:nparam,m+mp+mrecvleft(1))=recvright(1:nparam,m)
     zelectron0(1:nparam,m+mp+mrecvleft(1))=recvright(nparam+1:nzphase,m)
  enddo
