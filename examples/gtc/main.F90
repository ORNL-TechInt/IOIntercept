...
program GTC
  ...
  real(doubleprec) t1,tchargei,tchargee
...
!total cpu and wall clock time
  ...
  if(mype==0)then
     ...
     write(gtcout,*)timecpu(1:8) !---wj - ADDED FOR MORE ACCURATE OUTPUT

! Restart file info
     FileExit=1
     iorestart=345
     open(iorestart,file="FileExit.out",status="replace")
     write(iorestart,100) "FileExit=",FileExit
100  format(A9,i1)
     write(iorestart,"(A9,i5)")"irest   =",irest
     if(mod(irest+1,2)==0)then
        write(iorestart,101)"restart_dir1"
     else
        write(iorestart,101)"restart_dir2"
     endif
101  format(A12)
     close(iorestart)
